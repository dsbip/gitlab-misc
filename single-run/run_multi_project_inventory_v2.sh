...
#   project_id, provider_url_var, service_account_var, location
...
parse_targets_sh() {
  ...
  local pid="" prov_var="" sa_var="" loc="" legacy=0 scalar=0 has_idtok=0
  local seen=" "
  ...
  _pt_set_field() {  # $1=key $2=value
    case "$1" in
      ...
      id_token_var)            has_idtok=1 ;;   # no longer per project
      location)                loc="$2" ;;
  ...
  _pt_reset() { pid=""; prov_var=""; sa_var=""; loc=""; legacy=0; scalar=0; has_idtok=0; }
  ...
  _pt_emit() {
    ...
    if [[ "$has_idtok" -eq 1 ]]; then
      log "WARN: entry #${idx} (${pid}) sets id_token_var, which is no longer used per project; the pipeline-wide ID_TOKEN_VAR applies. Ignoring it."
    fi
    seen="${seen}${pid} "
    printf '%s\t%s\t%s\t%s\n' "$pid" "${prov_var:-WIF_PROVIDER_URL}" \
      "${sa_var:-WIF_SERVICE_ACCOUNT}" "$loc"
  }

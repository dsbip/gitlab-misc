...
#   * the role exists,
#   * the role's per-DAG permissions exactly match the config (adds missing,
...
# If Airflow already matches the config, nothing is changed.
#
...
#
#   custom_role_name:
#     roles:                 # Airflow permission actions (e.g. "can read")
#       - can read
#       - can edit
#     Dags_list:             # DAG ids the permissions apply to
#       - dag_one
#       - dag_two
...
# "edit" -> can_edit, "menu access" -> menu_access, etc. Each becomes a
# permission on resource DAG:<dag_id> for every DAG in Dags_list.
#
# Config (flags override env vars):
#   --config      / RBAC_CONFIG          YAML file. Default: airflow-rbac/rbac_config.yml
...
CONFIG_FILE="${RBAC_CONFIG:-airflow-rbac/rbac_config.yml}"
...
reconcile_role() {
  ...
  # Additions grouped by action.
  local -A add_by_action=() del_by_action=()
  local key act res
  for key in "${!desired[@]}"; do
    [[ -n "${current[$key]:-}" ]] && continue
    act="${key%%|*}"; res="${key#*|}"
    add_by_action["$act"]="${add_by_action[$act]:+${add_by_action[$act]} }$res"
  done
